...
const StatViewLine: FC<IStatViewLine> = ({ title, number, percent, animate }) => {
  return (
    <div className='flex items-center w-full border-b gap-x-1 last:border-none'>
      <div className='flex flex-col w-full'>
        <div className='flex items-center justify-between w-full text-xs '>
          <span className='block w-full overflow-hidden text-[10px] whitespace-nowrap overflow-ellipsis'>{title}</span>
        </div>
