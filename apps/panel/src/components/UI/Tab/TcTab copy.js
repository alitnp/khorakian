...
const TcTab = ({ tabs, activeTab, onChange }) => {
  //state
      ...
      <div className='flex flex-col w-full p-0 mb-6 overflow-x-auto overflow-y-hidden border shadow-inner bg-t-layer-bg-color sm:flex-row print:hidden'>
        {tabs.map((item, index) => (
          <div
            ...
            <span className={`flex items-center whitespace-nowrap gap-x-2 ${selectedTab === item.key && style['selected-text']}`}>
              {item.icon && item.icon}
