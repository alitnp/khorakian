import TcButton from 'components/UI/Button/TcButton';
import TcCard from 'components/UI/Card/TcCard';
import TcCoverLoading from 'components/UI/Loading/TcCoverLoading';
import TcDeleteIcon from 'components/UI/TableIcons/TcDeletIcon';
import ApiService, { BASE_URL, errorResponse } from 'config/API/ApiService';
import endpointUrls from 'global/Constants/endpointUrls';
import { handleApiThen } from 'global/helperFunctions/handleApiThen';
import useApiCatcher from 'global/helperFunctions/useApiCatcher';
import { signalFullDetail, signalImage } from 'global/Models/SignalModels';
import { FC, useRef, useState } from 'react';
import { useDispatch } from 'react-redux';
import { setNotificationData } from 'redux/reducer/Toast/toastReducer';

interface ISignalImageManage {
  signal: signalFullDetail | undefined;
  refetch(): void;
}

const SignalImageManage: FC<ISignalImageManage> = ({ signal, refetch }) => {
  //states
  const [loading, setLoading] = useState<boolean>(false);
  console.log(signal);
  //hooks
  const dispatch = useDispatch();
  const apiCathcer = useApiCatcher();

  //ref
  const fileInputRef = useRef<HTMLInputElement>(null);

  //constants
  const attachSizeLimit = 5;

  //functions
  const openFileInputWindow = () => {
    if (fileInputRef.current) {
      fileInputRef.current.value = '';
      fileInputRef.current.click();
    }
  };
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]?.size > attachSizeLimit * 1000000)
      return dispatch(setNotificationData({ message: `حداکثر حجم فایل قابل پیوست ${attachSizeLimit} مگابایت می باشد.`, type: 'warning' }));

    if (e.target.files && e.target?.files?.length > 0) handleFileUpload(e.target.files[0]);
  };

  const handleFileUpload = async (file: File) => {
    const form = new FormData();
    form.append('file', file);
    setLoading(true);
    await ApiService.post(endpointUrls.signalUploadImage, form, {
      'Content-Type': 'multipart/form-data',
    })
      .then((res: any) => {
        if (res.isSuccess) addImageToSignal(res.data);
        else {
          setLoading(false);
        }
      })
      .catch(() => {
        setLoading(false);
        apiCathcer(errorResponse);
      });
  };

  const addImageToSignal = async (imageName: string) => {
    await ApiService.post(endpointUrls.signalAddImage, { signalId: signal?.id, url: imageName })
      .then((res: any) => handleApiThen({ res, dispatch, onSuccess: refetch, notifFail: true, notifSuccess: true }))
      .catch(() => apiCathcer(errorResponse));
    setLoading(false);
  };
  const handleDeleteImage = async (image: signalImage) => {
    setLoading(true);
    await ApiService.post(endpointUrls.signalRemoveImage, { id: image.id, signalId: image.signalId, url: image.url })
      .then((res: any) => handleApiThen({ res, dispatch, onSuccess: refetch, notifFail: true, notifSuccess: true }))
      .catch(() => apiCathcer(errorResponse));
    setLoading(false);
  };

  return (
    <TcCard>
      <div className='flex items-center mb-4 gap-x-2'>
        <h2 className='whitespace-nowrap text-t-primary-color'>تعیین عکس ها</h2>
        <hr className='w-full' />
        <TcButton onClick={openFileInputWindow}>افزودن عکس</TcButton>
      </div>
      <input type='file' className='hidden' ref={fileInputRef} onChange={handleFileSelect} accept='image/*' />
      <div className='grid w-full grid-cols-5 gap-4'>
        {signal?.signalImages.map((image) => (
          <div className='overflow-hidden rounded-md shadow-md' key={image.id}>
            <div className='w-full aspect-square'>
              <img src={image.url} className='object-cover w-full h-full' />
            </div>
            <div className='flex items-center justify-center py-1'>
              <TcDeleteIcon onConfirm={() => handleDeleteImage(image)} />
            </div>
          </div>
        ))}
      </div>

      {loading && <TcCoverLoading />}
    </TcCard>
  );
};

export default SignalImageManage;
