...
export const parseStringArticle = (article: string) => {
	let content: any;
	try {
		content = JSON.parse(article);
		content.blocks = Object.values(content.blocks).map(
			(block: any) => {
				if (block.type === "image") {
					block.data.file.url =
						webConfig.domain + block.data.file.url;
				}
				return block;
			}
		);
