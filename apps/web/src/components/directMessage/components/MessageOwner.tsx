...
const MessageOwner: FC<ITcMessageOwner> = ({
	isActive,
	repeatedSender,
	fullName,
	creationTime,
		...
		>
			...
			>
				...
				{!repeatedSender && (
					<div
						className={`flex items-center justify-center w-10 h-10 text-xs font-bold rounded-full shadow-md text-t-opposite-text-color shrink-0   ${
							!isActive ? "bg-gray-600/40 " : "bg-green-700/40"
						}`}
					>
				...
				<div>
					{/* name */}
					{!repeatedSender && (
						<p className={`mb-1 ${!isActive && "text-left"}`}>
							{fullName}
						</p>
					)}
					{/* time */}
