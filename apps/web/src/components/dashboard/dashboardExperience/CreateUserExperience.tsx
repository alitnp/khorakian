...
import { ApiDataResponse, IUserExperience } from '@my/types';
...
const CreateUserExperience: FC<ICreateUserExperience> = ({
  ...
  const handleSubmit = async (values: any) => {
    setLoading(true);
    await WebApiService.post(webEndpointUrls.createUserExperience, values)
      .then((res: ApiDataResponse<IUserExperience>) =>
        webApiThen({
          res,
          notifFail: true,
          notifSuccess: true,
          onSuccess: () => push(webRoutes.dashboardExperience.path),
        })
      )
  ...
  return (
    <Modal open={visible} onCancel={close} footer={false} title="ثبت تجربه من">
      <div className="pt-6">
        <Form
          form={form}
